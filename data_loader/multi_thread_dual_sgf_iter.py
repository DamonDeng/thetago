...
class MultiThreadDualSGFIter(mx.io.DataIter):
    def __init__(self, sgf_directory, workers=1, batch_size=30, file_limit = -1, processor_class=FeatureProcessor, level_limit='0d', player='all'):
        ...
        self.batch_move_label = np.zeros(self.processor_class.get_label_shape_only(batch_size)[0])
        self.batch_value_label = np.zeros(self.processor_class.get_label_shape_only(batch_size)[1])
    ...
    def next(self):
        ...
        for i in range(self.batch_size):
            try:
              data, move_label, value_label = self.q.get(block=True, timeout=1)
            except queue.Empty:
              if not self.stop_q.empty():
                raise StopIteration
            self.batch_data[i] = data
            self.batch_move_label[i] = move_label
            self.batch_value_label[i] = value_label
        # print('got one batch of data')
        data = [mx.nd.array(self.batch_data)]
        label = [mx.nd.array(self.batch_move_label), mx.nd.array(self.batch_value_label)]
    ...
    for name in name_list:
        if name.endswith('.sgf'):
            ...
            for feature in features:
                ...
                data, move_label, value_label = feature
                output_q.put((data, move_label, value_label))
                if not stop_q.empty():
