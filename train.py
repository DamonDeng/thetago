...
def init(args):
    ...
    # empty iter created here to get the datashape

    data_iter = DumyIter(batch_size=64, history_length=7)
    net = NetworkFactory.getNetwork(args.network)
    run = TrainingRun.create(args.progress, corpus_index, net, data_iter)
...
def _prepare_training_data_single_process(worker_idx, chunk, corpus_index, output_q, stop_q):
    ...
    # print('preparing the data in thread:')
    # print(X.shape())
    # print(Y.shape())
...
def train(args):
    ...
    try:
        while True:
            ...
            # print(X[0])
            # print(Y[0])
            # print('Converting the X and Y into ND array !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            x_array = mx.nd.array(X)
            y_array = mx.nd.array(Y)

            # print(x_array.shape)
            # print(y_array.shape)

            # print('trying to build the iter here !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            nd_iter = mx.io.NDArrayIter(data={'data':x_array},
                             label={'softmax_label':y_array},
                             batch_size=64)
